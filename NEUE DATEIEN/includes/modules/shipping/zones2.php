 ...
 * @version $Id: zones2.php 734 2019-06-25 08:49:16Z webchills $
 */
/*

  USAGE
  By default, the module comes with support for 7 zones2.  This can be
  easily changed by editing the line below in the zones2 constructor
  that defines $this->num_zones2.
...
// class constructor
    function __construct() {
      ...
      $this->sort_order = defined('MODULE_SHIPPING_ZONES2_SORT_ORDER') ? MODULE_SHIPPING_ZONES2_SORT_ORDER : null;
      ...
      if (IS_ADMIN_FLAG === true) {
  		// build in admin only additional zones2 if missing in the configuration table due to customization of default $this->num_zones2 = 3
	  	  global $db;
  	  	for ($i = 1; $i <= $this->num_zones2; $i++) {
  		  	$check = $db->Execute("select * from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZONES2_COUNTRIES_" . $i . "'");
      ...
      if ($this->enabled) {
        global $db;
          ...
          $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_" . $i . "'");
          if ($check_query->EOF) {
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Handling Per Order or Per Box Zone " . $i . "  (when by weight)' , 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_" . $i."', 'Order', 'Do you want to charge Handling Fee Per Order or Per Box?', '6', '0', 'zen_cfg_select_option(array(\'Order\', \'Box\'), ', now())");
          }
        }
...
// class methods
    ...
    $order_total_amount = $_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices() ;
      ...
      if ($dest_zone == 0) {
        ...
        $done = false;
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_ZONES2_METHOD) {
        	  case (MODULE_SHIPPING_ZONES2_METHOD == 'Weight'):
              if (round($shipping_weight,9) <= $zones2_table[$i]) {
...
//                $shipping_method = MODULE_SHIPPING_ZONES2_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_ZONES2_TEXT_UNITS : '');
                ...
                $done = true;
        if (strstr($zones2_table[$i+1], '%')) {
          $shipping = ($zones2_table[$i+1]/100) * $order_total_amount;
        } else {
          $shipping = $zones2_table[$i+1];
        }
                break;
      	    ...
      	    break;
        	  case (MODULE_SHIPPING_ZONES2_METHOD == 'Price'):
// shipping adjustment
              if (($_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices()) <= $zones2_table[$i]) {
                $shipping = $zones2_table[$i+1];
        ...
        if (strstr($zones2_table[$i+1], '%')) {
          ...
          $shipping = $zones2_table[$i+1];
	          ...
	          break;
        	  case (MODULE_SHIPPING_ZONES2_METHOD == 'Item'):
// shipping adjustment
              if (($total_count - $_SESSION['cart']->free_shipping_items()) <= $zones2_table[$i]) {
                $shipping = $zones2_table[$i+1];
                $shipping_method = MODULE_SHIPPING_ZONES2_TEXT_WAY . ' ' . $dest_country;
                $done = true;
        if (strstr($zones2_table[$i+1], '%')) {
        ...
        if ($shipping == -1) {
          ...
          switch (MODULE_SHIPPING_ZONES2_METHOD) {
              ...
              if (constant('MODULE_SHIPPING_ZONES2_HANDLING_METHOD_' . $dest_zone) == 'Box') {
                $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ZONES2_HANDLING_' . $dest_zone) * $shipping_num_boxes;
              } else {
      ...
      if (strstr(MODULE_SHIPPING_ZONES2_SKIPPED, $dest_country)) {
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Versandkosten nach Zonen aktivieren?', 'MODULE_SHIPPING_ZONES2_STATUS', '43', 'Wollen Sie Versandkosten nach Zonen aktivieren?', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Berechnungsweise', 'MODULE_SHIPPING_ZONES2_METHOD', '43', 'Sollen die Kosten nach Gewicht (Weight), Preis (Price) oder pro Stück (Item) berechnet werden?', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Steuerklasse', 'MODULE_SHIPPING_ZONES2_TAX_CLASS', '43', 'Folgende Steuerklasse auf die Versandkosten anwenden:', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Basis der Steuern', 'MODULE_SHIPPING_ZONES2_TAX_BASIS', '43', 'Auf welcher Basis soll die Steuer für die Versandkosten berechnet werden. Mögliche Optionen:<br/>Shipping - basiert auf der Versandadresse des Kunden<br/>Billing - basiert auf der Rechnungsadresse des Kunden<br/>Store - basiert auf der Adresse des Shops falls Zone für Rechnung/Versand gleich der Zone des Shops ist.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Sortierreihenfolge', 'MODULE_SHIPPING_ZONES2_SORT_ORDER', '43', 'Anzeigereihenfolge. Niedrigste Werte werden zuerst angezeigt.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Länder ausschliessen', 'MODULE_SHIPPING_ZONES2_SKIPPED', '43', 'Tragen Sie hier nur die Länder ein, für die Sie Versandkosten nach Zonen NICHT anbieten wollen. Einzugeben mit den zweistelligen ISO Ländercodes durch Komma getrennt. z.B. US,DE,CH<br/>Wenn Sie keine bestimmten Länder ausschliessen wollen, lassen Sie dieses Feld leer!', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 1 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_1', '43', 'Tragen Sie hier die Länder für Ihre Zone 1 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 2 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_2', '43', 'Tragen Sie hier die Länder für Ihre Zone 2 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 3 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_3', '43', 'Tragen Sie hier die Länder für Ihre Zone 3 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 4 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_4', '43', 'Tragen Sie hier die Länder für Ihre Zone 4 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 5 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_5', '43', 'Tragen Sie hier die Länder für Ihre Zone 5 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 6 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_6', '43', 'Tragen Sie hier die Länder für Ihre Zone 6 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 7 Länder', 'MODULE_SHIPPING_ZONES2_COUNTRIES_7', '43', 'Tragen Sie hier die Länder für Ihre Zone 7 ein. Einzugeben sind die zweistelligen ISO Ländercodes durch Komma getrennt, z.B. DE,CH,AT<br/>Tragen Sie 00 ein, wenn die Zone für alle ISO Ländercodes gelten soll', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 1 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_1', '43', 'Versandkosten für die Länder der Zone 1<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 2 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_2', '43', 'Versandkosten für die Länder der Zone 2<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 3 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_3', '43', 'Versandkosten für die Länder der Zone 3<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 4 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_4', '43', 'Versandkosten für die Länder der Zone 4<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 5 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_5', '43', 'Versandkosten für die Länder der Zone 5<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 6 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_6', '43', 'Versandkosten für die Länder der Zone 6<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 7 Versandkosten', 'MODULE_SHIPPING_ZONES2_COST_7', '43', 'Versandkosten für die Länder der Zone 7<br/>Beispiel: 3:8.50,7:10.50,99:20.00<br/>Bedeutet: Gewicht/Preis kleiner oder gleich 3 kostet 8.50. Zwischen 3 und 7 kostet der Versand 10.50 usw.<br/>Sie können auch einen Prozentsatz der Bestellsumme als Versandkosten verwenden und z.B. definieren:<br/>25:8.50,35:5%,40:9.50,10000:7%', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 1 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_1', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 2 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_2', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 3 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_3', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 4 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_4', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 5 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_5', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 6 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_6', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 7 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES2_HANDLING_7', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 1 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_1', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 2 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_2', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 3 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_3', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 4 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_4', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 5 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_5', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 6 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_6', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 7 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_7', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
      ...
      for ($i = 1; $i <= $this->num_zones2; $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_ZONES2_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".<br />Set as 00 to indicate all two character ISO country codes that are not specifically defined.', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_ZONES2_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights/prices. Example: 3:8.50,7:10.50,... Weight/Price less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.<br />You can also use percentage amounts, such 25:8.50,35:5%,40:9.50,10000:7% to charge a percentage value of the Order Total', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_ZONES2_HANDLING_" . $i."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Handling Per Order or Per Box Zone " . $i . " (when by weight)' , 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_" . $i."', 'Order', 'Do you want to charge Handling Fee Per Order or Per Box?', '6', '0', 'zen_cfg_select_option(array(\'Order\', \'Box\'), ', now())");
    ...
    function keys() {
      ...
      for ($i=1; $i<=$this->num_zones2; $i++) {
        ...
        $keys[] = 'MODULE_SHIPPING_ZONES2_HANDLING_METHOD_' . $i;
      }
