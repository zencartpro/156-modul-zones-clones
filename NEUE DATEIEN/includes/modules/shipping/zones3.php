 ...
 * @version $Id: zones3.php 734 2019-06-25 08:49:16Z webchills $
 */
/*

  USAGE
  By default, the module comes with support for 7 zones3.  This can be
  easily changed by editing the line below in the zones3 constructor
  that defines $this->num_zones3.
  ...
  zones3.php.  A list of settings will appear to the right.  Click on the
  Edit button.
...
// class constructor
    function __construct() {
      ...
      $this->sort_order = defined('MODULE_SHIPPING_ZONES3_SORT_ORDER') ? MODULE_SHIPPING_ZONES3_SORT_ORDER : null;
      if (null === $this->sort_order) return false;

      $this->icon = '';
      $this->tax_class = MODULE_SHIPPING_ZONES3_TAX_CLASS;
      $this->tax_basis = MODULE_SHIPPING_ZONES3_TAX_BASIS;
      ...
      if (IS_ADMIN_FLAG === true) {
  		// build in admin only additional zones3 if missing in the configuration table due to customization of default $this->num_zones3 = 3
	  	  global $db;
  	  	for ($i = 1; $i <= $this->num_zones3; $i++) {
  		  	$check = $db->Execute("select * from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZONES3_COUNTRIES_" . $i . "'");
	  		  if ($this->enabled && $check->EOF) {
          $default_countries = '';
          $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_ZONES3_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".<br />Set as 00 to indicate all two character ISO country codes that are not specifically defined.', '6', '0', 'zen_cfg_textarea(', now())");
          $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_ZONES3_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights/prices. Example: 3:8.50,7:10.50,... Weight/Price less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.<br />You can end the last amount as 10000:7% to charge 7% of the Order Total', '6', '0', 'zen_cfg_textarea(', now())");
          $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_ZONES3_HANDLING_" . $i."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
          $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Handling Per Order or Per Box Zone " . $i . "  (when by weight)' , 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_" . $i."', 'Order', 'Do you want to charge Handling Fee Per Order or Per Box?', '6', '0', 'zen_cfg_select_option(array(\'Order\', \'Box\'), ', now())");
  		  	}
    		}
  	  } // build in admin only
      ...
      if ($this->enabled) {
        global $db;
          ...
          $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_" . $i . "'");
          if ($check_query->EOF) {
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Handling Per Order or Per Box Zone " . $i . "  (when by weight)' , 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_" . $i."', 'Order', 'Do you want to charge Handling Fee Per Order or Per Box?', '6', '0', 'zen_cfg_select_option(array(\'Order\', \'Box\'), ', now())");
          }
        }
...
//                $shipping_method = MODULE_SHIPPING_ZONES3_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_ZONES3_TEXT_UNITS : '');
                $shipping_method = MODULE_SHIPPING_ZONES3_TEXT_WAY . ' ' . $dest_country . $show_box_weight;
                $done = true;
...
// shipping adjustment
        ...
        if ($shipping == -1) {
          ...
          switch (MODULE_SHIPPING_ZONES3_METHOD) {
              ...
              if (constant('MODULE_SHIPPING_ZONES3_HANDLING_METHOD_' . $dest_zone) == 'Box') {
                $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ZONES3_HANDLING_' . $dest_zone) * $shipping_num_boxes;
              } else {
              ...
              $shipping_cost = ($shipping) + constant('MODULE_SHIPPING_ZONES3_HANDLING_' . $dest_zone);
            break;
        	  case (MODULE_SHIPPING_ZONES3_METHOD == 'Item'):
      ...
      if (strstr(MODULE_SHIPPING_ZONES3_SKIPPED, $dest_country)) {
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ZONES3_STATUS'");
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Calculation Method', 'MODULE_SHIPPING_ZONES3_METHOD', 'Weight', 'Calculate cost based on Weight, Price or Item?', '6', '0', 'zen_cfg_select_option(array(\'Weight\', \'Price\', \'Item\'), ', now())");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Tax Basis', 'MODULE_SHIPPING_ZONES3_TAX_BASIS', 'Shipping', 'On what basis is Shipping Tax calculated. Options are<br />Shipping - Based on customers Shipping Address<br />Billing Based on customers Billing address<br />Store - Based on Store address if Billing/Shipping Zone equals Store zone', '6', '0', 'zen_cfg_select_option(array(\'Shipping\', \'Billing\', \'Store\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_ZONES3_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Skip Countries, use a comma separated list of the two character ISO country codes', 'MODULE_SHIPPING_ZONES3_SKIPPED', '', 'Disable for the following Countries:', '6', '0', 'zen_cfg_textarea(', now())");
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 4 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES3_HANDLING_4', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 5 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES3_HANDLING_5', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 6 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES3_HANDLING_6', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 7 Bearbeitungsgebühr', 'MODULE_SHIPPING_ZONES3_HANDLING_7', '43', 'Wenn Sie in dieser Zone eine Bearbeitungsgebühr verrechnen wollen, hier eintragen. Falls nicht, die Null lassen.', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 1 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_1', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 2 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_2', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 3 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_3', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 4 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_4', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 5 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_5', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 6 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_6', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
			$db->Execute("insert into " . TABLE_CONFIGURATION_LANGUAGE   . " (configuration_title, configuration_key, configuration_language_id, configuration_description, date_added) values ('Zone 7 Art der Bearbeitungsgebühr (nur falls nach Gewicht berechnet wird)', 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_7', '43', 'Falls Sie in dieser Zone eine Bearbeitungsgebühr verrechnen, soll die pro Bestellung (Order) oder pro Paket (Box) verrechnet werden?', now())");
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_ZONES3_STATUS', 'MODULE_SHIPPING_ZONES3_METHOD', 'MODULE_SHIPPING_ZONES3_TAX_CLASS', 'MODULE_SHIPPING_ZONES3_TAX_BASIS', 'MODULE_SHIPPING_ZONES3_SORT_ORDER', 'MODULE_SHIPPING_ZONES3_SKIPPED');

      for ($i=1; $i<=$this->num_zones3; $i++) {
        $keys[] = 'MODULE_SHIPPING_ZONES3_COUNTRIES_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZONES3_COST_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZONES3_HANDLING_' . $i;
        $keys[] = 'MODULE_SHIPPING_ZONES3_HANDLING_METHOD_' . $i;
      }
